...
export default {
  ...
  actions: {
    async setupProvider({ commit }, { rawProvider, account }) {
      ...
      commit('connect', { provider, accounts, signer, rawProvider, contracts })
    },
    ...
    async toggle({ state: { rawProvider }, dispatch }) {
      if (rawProvider) {
        return dispatch('reset')
      }
      try {
        const rawProvider = await w3Modal.connect()
