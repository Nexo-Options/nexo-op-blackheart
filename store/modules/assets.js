...
export default {
  ...
  mutations: {
    ...
    unwrap: (state, event) => {
      const holding = state.list.find(
        x => x.id === event.args.tokenId && x.asset === event.asset
      )
      if (holding) {
        ...
        console.warn('No such Nexo contract', event, [...state.list])
      }
    }
  },
  actions: {
    async processUnwrap({ commit }, { event, asset }) {
      const block = await event.getBlock()
      commit('unwrap', { ...event, asset, block })
    },
    async process({ commit, rootState }, { event, asset }) {
      const { whETHv2, whBTCv2 } = rootState.connection.contracts
    ...
    async load({ dispatch, rootState }) {
      ...
      await Promise.all(WBTCWrap.map(event => dispatch('process', { event, asset: 'WBTC' })))
    ...
    async unwrap({ dispatch, rootState }, { asset, id }) {
      const wh = asset === 'ETH'
        ? rootState.connection.contracts.whETHv2
        : rootState.connection.contracts.whBTCv2
    ...
    async getWrapCost({ rootState }, { symbol, amount, period }) {
      const asset =
        symbol === 'ETH'
          ...
          ? rootState.connection.contracts.whBTCv2
          : null
      return asset.wrapCost(amount, period * 24 * 3600)
    },
