      ...
      .my-table-box
        table.my-table
          tr.my-table__header
            ...
            th Amount
            th Transaction Hash
          tr.my-table__line(v-for="claim in claimHistory" :key="claim.transactionHash")
            td
            ...
            td
              a.hash(:href="`https://etherscan.io/tx/${claim.transactionHash}`" target="_blank")
                {{ claim.transactionHash }}

      // Mobile version
      .my-table-box--mobile.text-left
        .my-table__line(v-for="claim in claimHistory" :key="claim.transactionHash")
          .token.bold.USDC
            | {{ claim.args.amount.format(6, 2) }} USDC
            .action-absolute
              .value.bold.claimed Claimed
