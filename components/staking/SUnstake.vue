...
export default {
  ...
  computed: {
    ...
    estimated() {
      ...
      if (sh && total) {
        ...
        return sh.sub(this.amount).mul(10000).div(total.sub(this.amount))
      }
    },
    lockedFor() {
      return this.lockedUntil
        ? Math.ceil((this.lockedUntil - Date.now()) / 3600000)
        : null
    }
  },
  components: {
  ...
  methods: {
    async withdraw() {
      try {
        await this.$store.dispatch('staking/withdraw', this.amount)
        this.amount = null
      } catch (err) {
...
.new-swap.margin-top-32
  .new-swap__box--top
    ...
    .token--big-title
      .token.WHITE
        big-number(:value="balances.sWHITE") WHITE
