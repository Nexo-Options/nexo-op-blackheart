    ...
    .holding-box
      .holding-box__mobile-menu
        ...
        .my-table-container__title(
          v-if="historyAssets.length > 0"
          :class="{active: mobileActive==1}"
          @click="mobileActive=1"
        )
          | History
      ...
      .my-table-container(
        ...
        :class="{'mobile-active': mobileActive==0}"
      )
        .my-table-container__title Active Holdings
        .my-table-box
          table.my-table
            tr.my-table__header
              th Size
              ...
              th To be received
              th Receive now
            tr.my-table__line(v-for="i in activeAssets" :key="i.id || i.asset")
              td
                .value.bold
                  big-number(:value="i.amount" :decimals="i.decimals") {{ i.whAsset }}
              td
                .token $ 
                  big-number(:value="i.strike" :decimals="8" :places="2")
              td
                .token $
                  big-number(:value="$store.state.pricer[i.asset]" :decimals="8" :places="2")
              td
                .value(
                  :class="{bad: i.priceDiff && i.priceDiff.isNegative(), good: i.priceDiff > 0}"
                )
                  | {{ i.priceDiff && i.priceDiff.isNegative() ? '-' : i.priceDiff && !i.priceDiff.isZero() ? '+' : '' }}$
                  big-number(:value="i.priceDiff.abs()" :decimals="8" :places="2")
              td {{ i.duration }}
              td
                .value.bold
                  big-number(:value="i.amount" :decimals="i.decimals") {{ i.asset }}
        ...
        // Mobile layout
        .my-table-box--mobile
          .my-table__line(v-for="i in activeAssets" :key="i.id || i.asset")
            ...
            .value-line.bold
              ...
              .value.bold(:class="{good: i.profit > 0}")
                ...
                span(v-else) 0 USDC
              .value.bold )
            button.button.primary(@click="$store.dispatch('assets/unwrap', i)") Receive
      ...
      .my-table-container(
        v-if="historyAssets.length > 0"
        :class="{'mobile-active': mobileActive==1}"
      )
        .my-table-container__title History
        .my-table-box
          table.my-table
            tr.my-table__header
              th Size
              ...
              th Executed At
              th Received
            tr.my-table__line(v-for="i in historyAssets" :key="i.id || i.asset")
              td
                .value.bold
                  big-number(:value="i.amount" :decimals="i.decimals") {{ i.whAsset }}
              td
              ...
              td
                .token $
                  big-number(:value="i.closePrice" :decimals="8" :places="2")
              td.value(:class="{ good: i.closePrice && i.closePrice.sub(i.strike).gt(0), bad: i.closePrice && i.closePrice.sub(i.strike).lt(0)}")
                ...
                big-number(:value="i.closePrice && i.closePrice.sub(i.strike).abs()" :decimals="8" :places="2")
        ...
        // Mobile layout
        .my-table-box--mobile
          .my-table__line(v-for="i in historyAssets" :key="i.id || i.asset")
            .token.bold(:class="i.asset")
              big-number(:value="i.amount" :decimals="i.decimals") {{ i.whAsset }}
            .separator-line
            .my-table-line__container
