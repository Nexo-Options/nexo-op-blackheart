...
export default {
  ...
  computed: {
    available() {
      ...
      const protect = total.mul(2).div(10) // reserve 20%
      const free = total.mul(8).div(10).sub(locked) // 80% available minus locked
  ...
  .new-swap__box--top
    ...
    .price-info
      .table-box-line__elem
        .table-box-elem__info Available to wrap:
      .table-box-line__elem
        .table-box-elem__info
          .token.bold(:class="from")
            big-number(:value="available" :places="2" :decimals="6") {{ from }}
    ...
    .table-box
      .table-box__line
        ...
        .table-box-line__elem
          ...
          .table-box-elem__info
            | $
            big-number(
              :value="price && price.mul($store.state.pricer[from]).div(period * 24)"
              placeholder="0"
              :decimals="from === 'ETH' ? 26 : 16"
              :places="2"
            )
      ...
      .table-box__line.big
        ...
        .table-box-line__elem
          .table-box-elem__info
            | $
            big-number(:decimals="8" :places="2" :value="$store.state.pricer[from]")
